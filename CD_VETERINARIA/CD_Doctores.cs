...
namespace CD_VETERINARIA
{
    public class CD_Doctores
    {
        ...
        public DataTable Mostrar()
        {
            ...
            comando.CommandText = "SELECT*FROM DOCTORES";//Instruccion a ejecutar
        ...
        public void Insertar(string nombre, string apellido, string especialidad)
        {
            comando.Connection = conexion.AbrirConexion();//Abrimos la conexion para poder hacer operaciones con nuestra database
            comando.CommandText = "INSERT INTO DOCTORES VALUES('" + nombre + "','" + apellido + "','" + especialidad + "')";//Ponemos las variables
        ...
        public void Editar(string nombre, string apellido, string especialidad, int id)//Metodo para editar un registro
        {
            comando.Connection = conexion.AbrirConexion();//Importante abrir la conexion !!!
            comando.CommandText = "UPDATE DOCTORES SET " + @"[NOMBRE] = @NOMBRE, [APELLIDO] = @APELLIDO,[ESPECIALIDAD]=@ESPECIALIDAD " + @" WHERE [ID] = @ID";
            comando.CommandType = CommandType.Text;//Tenemos que especificar de nuevo que usamos transact
        ...
        {
            comando.Connection = conexion.AbrirConexion();
            comando.CommandText = "DELETE FROM DOCTORES WHERE ID = @id";
            comando.CommandType = CommandType.Text;//Tenemos que especificar de nuevo que usamos transact
            comando.Parameters.AddWithValue("@id", id);
